...
$('#search').addEventListener(
  ...
  debounce(async e => {
    ...
    const res = await fetch(`/api/search?q=${encodeURIComponent(q)}`).then(r =>
      r.json()
    );
    if (!res.length) {
    ...
    results.innerHTML = res
      .map(x => `<div class="hit" data-path="${x.p}">🎬 ${x.name}</div>`)
      .join('');
  }, 300)
);
...
results.addEventListener('click', async e => {
  ...
  setbgBackdrop(movieName);

  // probe once and show options
  const info = await fetch(`/api/probe?path=${encodeURIComponent(p)}`).then(r =>
    r.json()
  );
  ...
  $('#transVideo').checked = !(
    info.video &&
    (info.video.codec === 'hevc' || info.format.avgMbps < 3)
  );
  // audio: show tracks
  const aSel = $('#audioIndex');
  aSel.innerHTML = (info.audio || [])
    .map(
      (a, i) =>
  ...
  $('#transAudio').checked = !(
    def &&
    (def.codec === 'aac' || def.codec === 'ac3' || def.codec === 'eac3')
  );
  ...
  // Show info text
  document.getElementById('bitrateInfo').textContent = `${avg} Mb/s → GQ ${cq}`;
  ...
  const c = avg < 2 ? 'limegreen' : avg < 4 ? 'gold' : avg < 6 ? 'orange' : 'crimson';
...
function startLogSSE(id) {
  ...
  es.onerror = () => {
    /* stream will end when job finishes; hide if needed */
  };
}
